...
def calcular_propina(monto: float, propina_pct: float, personas: int) -> str:   """
    ...
    if personas < 1:
        raise ValueError("ERROR CRÍTICO: El número de personas debe ser mayor que cero.")
...
class CalculadoraPropinas:
    ...
    def ejecutar(self) -> None:
        ...
        print(f"\n  💰 Bienvenido a la Calculadora de Propinas v{VERSION}")
        print(f"  💱 Moneda: {MONEDA_SIMBOLO} | Historial máx: {MAX_HISTORIAL}")
        print(f"  💾 {len(self.historial)} registro(s) cargado(s) desde disco.")
